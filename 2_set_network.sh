...
# disable /usr/bin/nmcli permission for openstack project
chmod a-x /usr/bin/nmcli
...
for device in $NET_IF;do
		...
		if [ -z $gw ];then
		nmcli connection add type ethernet ifname $dev con-name "$dev" ip4 "$ip/$mask" mtu $mtu
		else
...
for device in $BONDING;do
        ...
        if [ -z $IS_NetworkManager ];then
                if [ "O${mode}O" = "O2O" -o "O${mode}O" = "O4O" ];then
                        STRING+="DEVICE=\"$nick\"\nONBOOT=yes\nBOOTPROTO=static\nBONDING_OPTS=\"miimon=100 mode=$mode xmit_hash_policy=1\"\nIPADDR=$ip\nNETMASK=$mask\nGATEWAY=$gw\nMTU=$mtu\n"
                else
                ...
                STRING2+="ifenslave $nick $DEVS > /dev/null 2>&1\n"
        else
                if [ "O${mode}O" = "O2O" -o "O${mode}O" = "O4O" ];then
		...
		if [ -z $gw ];then
		nmcli con modify  $nick ipv4.address $ip/$mask mtu $mtu ipv4.method manual connection.autoconnect-slaves 1
		else
...
if [ -z $IS_NetworkManager ];then
        ...
        sed -r -i "1a\\$STRING2$STRING3" /etc/rc.d/rc.local
else
